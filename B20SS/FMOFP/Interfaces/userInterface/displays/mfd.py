...
class MultiFunctionDisplay(BaseDisplay):
    ...
    def draw_mission_page(self, painter: QPainter, rect: QRectF):
        """Draw mission page — live data from MissionService."""
        try:
            mission = {}
            try:
                from FMOFP.Systems.missionPlanning.missionService import get_mission_service
                mission = get_mission_service().get_data()
            except Exception:
                pass
            ...
            painter.drawText(QRectF(rect.left() + 6, y, rect.width() - 12, 22),
                             Qt.AlignmentFlag.AlignVCenter, f"PHASE: {phase}")
            y += 26
            ...
            def row(label, value):
                ...
                painter.setFont(f_hd)
                painter.setPen(QPen(dim))
                painter.drawText(QRectF(rect.left() + 10, y, rect.width() * 0.45, line_h),
                                 Qt.AlignmentFlag.AlignVCenter, label)
                painter.setFont(f_val)
                painter.setPen(QPen(green))
                painter.drawText(QRectF(rect.left() + rect.width() * 0.47, y,
                                        rect.width() * 0.50, line_h),
                                 Qt.AlignmentFlag.AlignVCenter | Qt.AlignmentFlag.AlignRight, value)
                y += line_h + 1
            ...
            if targets:
                ...
                for tgt in targets[:3]:
                    ...
                    tcol = green if status == "DESTROYED" else amber if status == "ENGAGED" else cyan
                    painter.setFont(f_val)
                    painter.setPen(QPen(tcol))
                    name = tgt.get("name", f"TGT{tgt.get('id', 0)}")
                    painter.drawText(QRectF(rect.left() + 6, y, rect.width() - 12, line_h),
                                     Qt.AlignmentFlag.AlignVCenter,
                                     f"  {name:<10} [{status}]")
                    y += line_h + 1
